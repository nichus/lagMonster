...
var interaction_iterations = 0;
const interactions = [
  [
    new RegExp(/(ask jake|ask ripper|Baby Yoda|eat shit|I have spoken|I just buy what he says|I just buy what jake says|I just buy what ripper says|jake knows|jake probably knows|jake would know|like jake said|like ripper said|Mando|Nick Nolte|ripper knows|ripper probably knows|ripper would know|what jake said|what jake says|what ripper said|what ripper says)/),
  ...
  [
    new RegExp(/bsod|blue screen of death|blue screen|bsod'd|epic fail/),
...
function interact(message) {
  ...
  if (interaction_iterations == 0) {
    interactions.sort((a,b) => { Math.floor(Math.random()*3) - 1 });
  }

  found = interactions.find(e => { return message.content.match(e[0]) });
  if (!found) {
    return;
  }
