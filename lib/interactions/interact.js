...
const interactions = [
	[
		...
		{ 'image': 'https://lh3.googleusercontent.com/pw/ADCreHetmkUJ1FFAPSVmxEAdT2VMfcg_jKW2ryHDhTYKpqkFtX6MTKi5UhVAYH66UkApLJxgk0KBUfCeuHZ3knIGblb8eTS34zhzGJtKQkw2nSShuNWvOzc' },
	],
	[
	...
	[
		new RegExp(/bakhara nda|aged yellow scroll|pee scroll/i),
...
function pickField(field, value) {
	if (value in field) {
		if (Array.isArray(field[value])) {
			...
			// logger.debug(`Can't interact with a ${typeof fieldValue} type element.`);
			// logger.debug(found);
		}
	}
	return false;
...
function interact(message) {
	...
	/*
	const sticker = pickField(found[1],'sticker');
	if (sticker) {
		response.setSticker(
	*/
